...
if (-not (Test-UnderDirectory $TargetSkill $SkillsRoot)) {
	throw "Refusing to modify target outside project skills directory: $TargetSkill"
}
...
if ($Copy) {
	...
	Write-Host "Copied Cursor skill from $SourceSkill to $TargetSkill"
} else {
	New-Item -ItemType Junction -Path $TargetSkill -Target $SourceSkill | Out-Null
	Write-Host "Linked Cursor skill $TargetSkill -> $SourceSkill"
}
