...
TSharedPtr<FJsonObject> FExecPythonAction::ExecuteInternal(const TSharedPtr<FJsonObject>& Params, FMCPEditorContext& Context)
{
	...
	FString WrapperCode = FString::Printf(TEXT(
		...
		"import collections.abc as __mcp_abc\n"
		"import traceback as __mcp_traceback\n"
		"\n"
		"def __mcp_to_jsonable(__mcp_obj):\n"
		"    if __mcp_obj is None or isinstance(__mcp_obj, (str, int, float, bool)):\n"
		"        return __mcp_obj\n"
		"    if isinstance(__mcp_obj, (bytes, bytearray)):\n"
		"        return __mcp_obj.decode('utf-8', errors='replace')\n"
		"    try:\n"
		"        import unreal as __mcp_unreal\n"
		"        if isinstance(__mcp_obj, __mcp_unreal.Object):\n"
		"            __mcp_class = __mcp_obj.get_class()\n"
		"            return {\n"
		"                'path': __mcp_obj.get_path_name(),\n"
		"                'name': __mcp_obj.get_name(),\n"
		"                'class': __mcp_class.get_name() if __mcp_class else ''\n"
		"            }\n"
		"    except Exception:\n"
		"        pass\n"
		"    if isinstance(__mcp_obj, __mcp_abc.Mapping):\n"
		"        return {str(__mcp_to_jsonable(__mcp_k)): __mcp_to_jsonable(__mcp_v) for __mcp_k, __mcp_v in __mcp_obj.items()}\n"
		"    if isinstance(__mcp_obj, (list, tuple, set)):\n"
		"        return [__mcp_to_jsonable(__mcp_item) for __mcp_item in __mcp_obj]\n"
		"    try:\n"
		"        __mcp_iter = iter(__mcp_obj)\n"
		"    except Exception:\n"
		"        return str(__mcp_obj)\n"
		"    return [__mcp_to_jsonable(__mcp_item) for __mcp_item in __mcp_iter]\n"
		"\n"
		...
		"        try:\n"
		"            __mcp_return_value_json = __mcp_json.dumps(__mcp_to_jsonable(__mcp_return_value), ensure_ascii=False)\n"
