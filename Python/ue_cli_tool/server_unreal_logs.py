...
def _infer_workspace_root() -> Path | None:
    ...
    # Plugin-relative fallback: .../Plugins/UEEditorMCP/Python/ue_cli_tool
...
def _read_saved_logs(
    ...
    if not project_log_dir.exists():
        return {
            ...
            "notes": notes + ["闇€瑕佹彁渚涙纭殑 workspaceRoot锛屼笖鐩綍涓寘鍚?Saved/Logs"],
        }
    ...
    if (
        ...
        if digest != current_digest:
            notes.append("cursor 鎸囧悜鏃ф棩蹇楁枃浠讹紝宸查噸缃埌鏈€鏂版枃浠?)
        elif stat.st_size < offset:
            notes.append("妫€娴嬪埌鏃ュ織杞浆鎴栨埅鏂紙size < offset锛夛紝宸查噸缃?cursor")
        elif stat.st_mtime_ns != cursor_mtime_ns and stat.st_size == cursor_size:
            notes.append("鏃ュ織鍏冧俊鎭彉鍖栵紝鎸夊綋鍓嶄綅缃户缁鍙?)
        else:
            incremental = True
...
def _handle_unreal_logs_get(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if mode == "live":
        if not ue_reachable:
            return {
                "success": False,
                "error": "UE 涓嶅彲杈撅紝mode=live 鏃犳硶璇诲彇瀹炴椂缂撳啿鍖?,
                "notes": ["鍙敼鐢?mode=saved 骞舵彁渚?workspaceRoot 璇诲彇 Saved/Logs"],
            }
    ...
    if mode == "auto" and ue_reachable:
        ...
        if live_result.get("success"):
            ...
            return live_result
        notes.append(
            f"live 涓嶅彲鐢紝鍥為€€鍒?Saved/Logs: {live_result.get('error', 'unknown')}"
        )

    if workspace_root is None:
        return {
            "success": False,
            "error": "鏃犳硶鎺ㄦ柇 workspaceRoot锛屼笖鏈樉寮忔彁渚涖€?,
            "notes": notes + ["璇蜂紶鍏?workspaceRoot锛屼緥濡?C:/work/SVN/p110_2"],
        }
...
def _handle_asset_diff_get(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if not _is_ue_reachable():
        return {
            "success": False,
            "error": "UE 涓嶅彲杈撅紝鏃犳硶鎵ц Diff Against Depot",
            "notes": [
                "璇峰厛鍚姩 Unreal Editor 骞剁‘淇?UEEditorMCP 杩炴帴鍙敤锛屼笖 Source Control 宸茶繛鎺?
            ],
        }
    ...
    if not isinstance(asset_path, str) or not asset_path.strip():
        return {
            "success": False,
            "error": "缂哄皯蹇呭～鍙傛暟 assetPath",
            "notes": [
                "璇锋彁渚涘畬鏁磋祫浜ц矾寰勶紝濡?/Game/P110_2/Blueprints/Character/Player/BP_SideScrollingCharacter"
            ],
        }
...
def _handle_asset_thumbnail_get(args: dict[str, Any]) -> dict[str, Any]:
    if not _is_ue_reachable():
        return {
            "success": False,
            "error": "UE 涓嶅彲杈撅紝鏃犳硶璇诲彇缂栬緫鍣ㄨ祫浜х缉鐣ュ浘",
            "notes": ["璇峰厛鍚姩 Unreal Editor 骞剁‘淇?UEEditorMCP 杩炴帴鍙敤"],
        }
...
server = Server("ue-cli-tool-logs")
...
async def _run() -> None:
    logger.info("Starting ue-cli-tool-logs server")
...
def main() -> None:
    ...
    try:
        ...
        logger.info("ue-cli-tool-logs stopped by user")
