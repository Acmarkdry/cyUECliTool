        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "Optional keyword filter on name/description",
                    },
                    "limit": {
                        "type": "number",
                        "description": "Max items to return (default 60)",
                    },
                    "include_schema": {
                        "type": "boolean",
...
async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
    ...
    if name == "ping":
        ...
        return [
            TextContent(
                type="text",
                text=f'{{"success": {str(success).lower()}, "pong": {str(success).lower()}}}',
            )
        ]
    ...
    if name == "invoke_tool":
        ...
        if not target_name:
            return [
                TextContent(
                    type="text",
                    text='{"success": false, "error": "tool_name is required"}',
                )
            ]
        if target_name in {"invoke_tool", "discover_tools"}:
            return [
                TextContent(
                    type="text",
                    text='{"success": false, "error": "invoke_tool cannot recursively invoke meta tools"}',
                )
            ]
        return await call_tool(
            str(target_name), target_args if isinstance(target_args, dict) else {}
        )
    ...
    return [
        TextContent(
            type="text", text=f'{{"success": false, "error": "Unknown tool: {name}"}}'
        )
    ]
